function step3(stepDto) {
    ...
    for(let i = 0; i<stepDto.contractors.length; i++){
        let contractor = stepDto.contractors[i];
        $('#contractor_list').append('<tr><td data-toggle="modal" data-target="#contractor_modal">'+ contractor.name +'</td>' +
            '<td><a class="btn btn-warning" onclick="showContractor()">Данные</a></td><td><div class="dropdown"><input id="datepicker3" width="276" />' +
            '<button class="btn btn-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="1" onclick="getData(contractor.id)">Назначить встречу' +
